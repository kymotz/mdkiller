...
1、引入依赖，由于是单文件，直接把文件拷贝到项目即可即可使用。
...
- 鲁棒性：具备一定容错行，所有接口均经过单元测试，见 `test/java/com/elltor/md`。

## 三、最佳实践

在 IM 中排版消息是痛苦的，这个工具就是要解决这个问题。通常在 IM 排版消息会遇到下列问题：

1. 在程序中直接拼接很难对 Markdown 进行排版，拼接使人头大，维护更加麻烦。（P.S. 不反对使用模版，但当我们增加/删除模版中的参数时也是麻烦的）
2. 差的 Markdown 消息特点：文案没有结构、重点不突出、交互差，不能突出主题。

因此，我们改善 IM 消息实际就是解决上面两个问题。
...
Bug 可以直接提 Issue，使用建议、交流 👉 `elltor(at)163.com`。
